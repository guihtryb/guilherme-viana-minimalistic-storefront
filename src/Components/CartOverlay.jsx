...
class CartOverlay extends Component {
  ...
  showHideCart() {
    ...
    for (let main of mainTag) {
      main.classList.contains('blurr') ? main.classList.remove('blurr')
      : main.classList.add('blurr');
    }
  }
  ...
  renderCartItems() {
    ...
    return (
      <div className="cart-items">
        <h3 className="my-bag">My Bag, <span className="bag-items-quantity" data-testid="bag-items-quantity">{items.length} { items.length > 1 ? "items" : "item"}</span></h3>
        {
            ...
            <div className="cart-item" key={index}>
              ...
              <div className="cart-item-quantity-and-image">
                <div className="item-quantity-control-container">
                  <button className="item-quantity-control-btn" data-testid="increase-quantity-btn" onClick={ () => this.increaseQuantity(item) }>
                    +
                  </button>
                  <span data-testid="item-quantity">
                    {item.quantity}
                  </span>
                  <button className="item-quantity-control-btn" onClick={ () => this.decreaseQuantity(item) }>
                    -
                  </button>
                </div>
        ...
        <div className="total-price-container">
          ...
          <span className="total-price-value">{currenciesObject[currency]} {totalPrice.toFixed(2)}</span>
        </div>
  ...
  render() {
    ...
    return (
      <div className="cart-overlay-container" data-testid="cart-icon">
        <div className="items-length" style={ items.length > 0 ? null : {display: "none"}}>
